...
static __init int pchar_init(void) {
    ...
    pdevice = device_create(pclass, NULL, devno, NULL, "pchar%d", 5);
    if(IS_ERR(pdevice)) {
...
static long pchar_ioctl(struct file *pfile, unsigned int cmd, unsigned long param) {
    ...
    case FIFO_INFO:
        ...
        int ret = copy_to_user((void*)param, &info, sizeof(info_t));
        break;
    case FIFO_RESIZE:
        ...
        int ret1 = kfifo_out(&buf,buf1,len);
        kfifo_free(&buf);
        if(kfifo_alloc(&buf,param, GFP_KERNEL))
        {
            printk(KERN_ERR "%s: kfifo_alloc() failed.\n", THIS_MODULE->name);
            kfree(buf1);
            return -ENOMEM;
        }
        kfifo_in(&buf,buf1,len);
        kfree(buf1);
        break;
    default:
