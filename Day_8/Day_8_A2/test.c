...
#include "pchar_ioctl.h"  // Your custom ioctl definitions
...
int main(int argc, char *argv[])
{
    ...
    else if (strcmp(argv[1], "resize") == 0)
    {
        ...
        // Convert the third argument to a long integer using strtol
        long new_size = strtol(argv[2], NULL, 10);
        if (new_size <= 0)
        {
            printf("invalid new size.\n");
            _exit(2);
        }
        // Perform the resize operation using ioctl
        ret = ioctl(fd, FIFO_RESIZE, new_size);
        if (ret != 0)
            perror("ioctl() failed");
        else
            printf("fifo resized to %ld bytes.\n", new_size);
    }
    else
    {
        printf("invalid usage.\n");
